...
ENV LIBC_CFLAGS '-O2 -pipe -g' $libc_flags
ADD musl-src /musl-libc
ENV NPROC=4
RUN cd musl-libc && CFLAGS=$LIBC_CFLAGS ./configure --prefix=/usr && \
	make -j$NPROC && src/stdatomic/redefine_syms.sh
...
ENV SNEK_DEPS='zlib-dev libffi-dev libjpeg libjpeg-turbo-dev openblas-dev lapack-dev'
RUN apk add $SNEK_DEPS
ADD apps/cpython /apps/cpython
RUN cd apps/cpython && CFLAGS="-g" ./configure --enable-optimizations --with-lto && make && make install
...
RUN python3 -m pip install $SNEKSIT_PYDEPS
...
RUN python3 -m pip install $CLOBBER_PMDK_PYDEPS
# pmdk
