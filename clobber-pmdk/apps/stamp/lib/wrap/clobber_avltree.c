...
void nvm_ptr_record(void *ptr, size_t size){
   ...
   memcpy((void*)(ctx->funcPtr+pertx_counter), ptrint, size); 
...
void* init_runtime() {
    init_admin_pop();
    pop = pmemobj_open("/mnt/pmem/atree.pop", POBJ_LAYOUT_NAME(ATREE));
    if (pop == NULL) {
        pop = pmemobj_create("/mnt/pmem/atree.pop", POBJ_LAYOUT_NAME(ATREE), PMemSize, 0666);
    }
...
void tx_commit(ThreadContext *ctx) {
    ...
    memcpy(ctx->funcPtr, &valid, sizeof(uint8_t));
//    pmemobj_memcpy_persist(pop, ctx->funcPtr, &valid, sizeof(uint8_t));         
