...
void create_context(pthread_t self) {
    ...
    for (uint64_t i = 0; i < MaxThreads; i++) {
	    ...
	    ctx->v_Buffer = malloc(funcPtrSize);
...
static uint64_t __lock_val = 0;

void t_lock(){
		ThreadContext *ctx = my_context();
		assert(ctx->locksHeld == 0);
		while (__lock_val != 0 || __sync_lock_test_and_set(&__lock_val, 1)) _mm_pause();
        if (ctx->openTxs == 0) {
            tx_open(ctx);
            ctx->openTxs++;
        }
...
void t_unlock(){
	...
	__sync_lock_release(&__lock_val);
	assert(ctx->locksHeld == 0);
}
