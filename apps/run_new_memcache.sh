...
printf "Running Zhuque & native memcached 1.6.17..."
{
for T in $THREADS; do
	for W in $WORKLOAD; do
		for C in $CONFIGS; do
			for I in `seq 0 4`; do
				...
				if [[ $C == 'zhuque' ]]; then
					rm -rf $ZHUQ_DIR/*
					LD_RELOAD=$ZHUQ_DIR ./memcached-1.6.17/memcached -u root -p 11215 -l 127.0.0.1 -t $T &
				else # native
					./memcached-1.6.17/memcached -u root -p 11215 -l 127.0.0.1 -t $T &
				fi
				sleep 1
				cd $MNEM_DIR
				...
				echo "${C},${T},${W},${I},${RATE}">> memcached_new-$C.csv
			done
		done
	done
done
