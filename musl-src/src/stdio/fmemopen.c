...
static FILE* fmemopen_impl (struct mem_FILE* f, void* restrict buf, size_t size, const char* restrict mode) {
	...
	if (buf == (void*) ((char*) f + sizeof(*f) + 1)) f->f.flags |= F_INPLACE;
...
FILE* fmemopen_inplace (void** restrict bufp, size_t size, const char* restrict mode) {
	...
	struct mem_FILE* f = (struct mem_FILE*) *bufp;
	size_t space = sizeof(struct mem_FILE) + 1;
	...
	size -= space;
	*bufp = (void*) (((char*) *bufp) + space);
	*(((char*) (*bufp)) - 1) = 0;
	return fmemopen_impl(f, *bufp, size, mode);
}
...
FILE *fmemopen(void *restrict buf, size_t size, const char *restrict mode)
{
	...
	if (!f) return 0;
	return fmemopen_impl(f, buf, size, mode);
}
