...
_Noreturn void __pthread_exit(void *result)
{
	...
	if (state==DT_DETACHED && self->map_base) {
		...
		_p_unmapself(self->map_base, self->map_size);
	}
...
#ifdef USE_VSTACK
#define THRSTACKFLAGS (MAP_REAL_ANON|MAP_PRIVATE|MAP_ANON|MAP_STACK)
#else
#define THRSTACKFLAGS (MAP_PRIVATE|MAP_ANON|MAP_STACK)
#endif
		map = __mmap(0, size*STACK_BLOCKOUT_FACTOR, PROT_NONE, THRSTACKFLAGS, -1, 0);
		if (map == MAP_FAILED) goto fail;
		if (__mprotect(map + size*(STACK_BLOCKOUT_FACTOR-1), size, PROT_READ|PROT_WRITE) && errno != ENOSYS) {
			__munmap(map, size*STACK_BLOCKOUT_FACTOR);
			goto fail;
		}
		tsd = map + size*STACK_BLOCKOUT_FACTOR - __pthread_tsd_size;
		stack = tsd - libc.tls_size;
	  	stack_limit = map + size*(STACK_BLOCKOUT_FACTOR-1);	

		new = __copy_tls(tsd - libc.tls_size);
		new->map_base = map;
		new->map_size = size*STACK_BLOCKOUT_FACTOR;
		new->stack = stack;
		new->stack_size = stack - stack_limit;
		new->guard_size = size*(STACK_BLOCKOUT_FACTOR-1);
		new->self = new;
		new->tsd = (void *)tsd;
...
#ifdef TIME_PSYS
	TIMER_START();
	int s =  _p_pthread_create(res, attrp, entry, arg);
	TIMER_STOP("pthread_create()");
	return s;
#else
	return _p_pthread_create(res, attrp, entry, arg);
#endif
}
