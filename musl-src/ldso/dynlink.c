...
static void psys_sync () {
	_p_save_loader_meta(head, tail, fini_head, syms_tail, lazy_head);
}
...
void __dls3(size_t *sp, size_t *auxv)
{
	...
	psys_sync();
		...
		// have to redo decoding because this dso is on the stack
		//decode_dyn(&app);
		_p_restore_loader_meta(&head, &tail, &fini_head, &syms_tail, &lazy_head);
	}
...
void *dlopen(const char *file, int mode)
{
	...
	if (!file) return (mode & RTLD_GETRTLD) ? &ldso : head;
